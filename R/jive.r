jive <- function (data, rankJ=1, rankA=rep(1,length(data)), method="perm", init = -1, scalefloat = 1000, dnames=names(data), conv="default", maxiter=1000, scale=TRUE, center=TRUE,orthIndiv=TRUE, est=TRUE, showProgress=TRUE, impute_miss = FALSE) {
   ...
   for (i in 1:l) {
     ...
     if (scale){
      ...
      data[[i]] <- data[[i]] / scaleValues[i] * scalefloat
     }
...
jive.iter = function (data, rankJ=1, rankA=rep(1,length(data)), conv=0.000001, maxiter=1000, orthIndiv=TRUE, showProgress=TRUE) {
  ...
  Jtot <- matrix(init, nrow(Xtot), ncol(Xtot))
  Atot <- matrix(init, nrow(Xtot), ncol(Xtot))
  ...
  while (nrun < maxiter & !converged) {
    ...
    timeA <- system.time({
      ...
      for (i in 1:l) {
        ...
        if (rankA[i] > 0) {
          ...
          time1 = Sys.time()
          s <- trunc_svdwrapper(temp, nu=rankA[i], nv=rankA[i])
          time2 = Sys.time()
          print(time2-time1)
    ...
    if (orthIndiv & nrun == 0) {
      ...
      for(i in 1:l){     ########re-estimate the Vind's
        if (rankA[i] > 0) {
          time1 = Sys.time()
          s <- trunc_svdwrapper(A[[i]], nu=rankA[i], nv=rankA[i])
          time2 = Sys.time()
          print(time2-time1)
...
trunc_svdwrapper = function(x, nu, nv, verbose=F ){
  # workaround by Art Owen to avoid LAPACK errors
  # See https://stat.ethz.ch/pipermail/r-help/2007-October/143508.html  
  gotit = F
  k = min(nu,nv)
  try( {svdx = svds(x,k,nu,nv); gotit=T}, silent = !verbose )
  if( gotit )return(svdx)
  try( {svdtx = svds(t(x),k,nv,nu); gotit=T}, silent = !verbose )
